  ...
  {  
    ...
    { 
      ...
      // XTE in feet computed in GPS_JNE_V# in feet + means right of track, steer left 
       ...
       // NEW WAYPOINT   steer to waypoint bearing until steadied up after turn to prevent oversteering. 
           // remove this piece if I get anticipatory steering using $GPRTE
       if (Waypoint_old != Waypoint_next)
         { 
           new_waypoint = true;
           waypoint_timer = millis();
           Waypoint_old = Waypoint_next;
         }
       if (millis() - waypoint_timer > new_waypoint_time_delay * 1000)
        {
          new_waypoint = false;
        }
       if(new_waypoint)
      ...
      else  GPS_Steering = false; // HTS is unchanged and boat will steer current compass HTS    
    }  // end void GPS_Steer()
 /*************************************************************************************/   
 void Knob_Steering()
 {
