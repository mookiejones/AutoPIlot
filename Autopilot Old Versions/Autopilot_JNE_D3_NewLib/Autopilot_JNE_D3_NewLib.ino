...
// AP C4 In GPS Steer make default on waypoint arrival to continue on course with auto turning to new course.
// AP D extension of C4. Added in MSG to eztransfer to send a message ID compactly to display messages on TFT
// AP D3 small changes
// AP D3_NewLib  Pololu updated library for IMU V3 and chaged MiniIMU9AHRS software so I had to update code to include version dated 2/5/15
// AP D3_NewLib  installed new Pololu Libraries took D3 deleted #include <LSM303.h> and #include <L3G.h>, reimported these two libraries then cut and pasted the
//     new insertions to current location.   Separately opened New version of MiniImu9AHRS (2/5/15) for each of the following Tabs I deleted the code from 
//   vD3 and pasted in the code from the corresponding code MinIMU9AHRS (for Tabs Compass, DCM, I2C, Output< Vector, matrix)
...
/**************USER INPUT****************/
 #define Motor_Controller 1  // 1 for Pololu Qik dual controller, 2 for Pololu Trex dual controller
// float PID_Ks[4] = {.75,.4,.01,0}; // use with rudder_command = rudder_command  + PID_output;
 ...
 float Kxte[3] = {.2, 4, .0004}; // used for XTE PID, 0 is proportional, 1 is differential, 2 is integral error, see GPS_Steer() in PID
 ...
 int new_waypoint_time_delay = 60;
 ...
 boolean Serial_Remote_Is_Connected = 1; // 1 remote connected, 0 remote not connected Pgm hangs up if remote data is sent and remote is not connected
