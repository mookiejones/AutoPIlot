...
#define RF24_Attached 1 // 0 if RF 24 radio modules are not used, 1 if they are used
...
char KeyIn2; // to test key data from other radio
 ...
 /*    struct RF_DATA1
     {
      int RFdata_set; // used to destinguish between RFdata1 and RFdata2 or others
      float course;
    ...
    RF_DATA1 RFdata1; //NAME THE DATA STRUCTURE
...
// float GPS_PID = 0;
 ...
 //  Motor controll parameters, 1 for Pololu Qik Dual controller 
   ...
   int Motor_1_rev = 143;
    int   motorspeedMIN = 0; // this value is the minimum speed sent to controller if left or right rudder is commanded
                            //  rudde stop will still send 0. Use to overcome starting torque
...
*/  
 ...
 void setup() {
  ...
  // pinMode(19, INPUT); // these were for Serial remote
  // pinMode(18, OUTPUT);
 //  pinMode(52, OUTPUT); // Green LED
   pinMode(48, INPUT); //SW1
   pinMode(46, INPUT); //SW2
  // pinMode(10, OUTPUT); // Hydraulic Solenoid Valve  These defs were for my old Wagener autopilot motor controller
...
#if RF24_Attached == 1
/** Radio Setup  **/
  fdevopen( &serial_putc, 0 ); // for printf // for RF printf support for if available 
  radio.begin();   
    radio.openWritingPipe(pipes[0]);
    radio.openReadingPipe(1,pipes[1]);
  int dataSize = sizeof(RF_DATA);
  printf("Size of RF_DATA1: %d\n", dataSize);
  if ( dataSize > 32 )
    printf("*** RF_DATA1 struct is too large ***\n");
